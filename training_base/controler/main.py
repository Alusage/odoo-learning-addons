...
class WebsiteEventController(http.Controller):
    ...
    def events(self, page=1, **searches):
        ...
        searches.setdefault('training_id', 0)
        searches.setdefault('is_training', 'True')
        searches.setdefault('idf', 0)
        ...
        if searches.get('is_training') == 'True':
            domain_search["training_id"] = ['|',('training_id', '=', int(searches.get('training_id'))), ('training_id.idf', '=', int(searches.get('idf')))]
        ...
        pager = request.website.pager(
            url="/event",
            url_args={'date': searches.get('date'), 'type': searches.get('type'), 'country': searches.get('country'), 'training_id': searches.get('training_id')},
            total=event_count,
            page=page,
            step=step,
            scope=5)
        ...
        _logger.debug("Domain: %s" % dom_without("none"))
        events = Event.search(dom_without("none"), limit=step, offset=pager['offset'], order=order)
        _logger.debug("Search path: ?%s" % werkzeug.url_encode(searches))
        values = {
