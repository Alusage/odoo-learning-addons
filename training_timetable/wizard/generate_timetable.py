...
class GenerateSession(models.TransientModel):
    ...
    batch_id = fields.Many2one('learning.batch', 'Group', required=True)
            ...
            for n in range((end_date - start_date).days + 1):
                ...
                for line in session.time_table_lines:
                    if int(line.day) == curr_date.weekday():
                        ...
                        self.env['learning.timesession'].create({
...
class GenerateSessionLine(models.TransientModel):
    ...
    teacher_id = fields.Many2one('learning.teacher', 'teacher', required=True)
    subject_id = fields.Many2one('learning.subject', 'Subject', required=True)
    timing_id = fields.Many2one('learning.timing', 'Timing', required=True)
    classroom_id = fields.Many2one('learning.classroom', 'Classroom')
